...
/**
 * MedPulseApp
 * Main client-side application controller for the standalone medical chat interface.
 * Implements conversation persistence, multi-turn history search, Markdown parsing,
 * syntax highlighting, and responsive drawer toggles.
 */
class MedPulseApp {
    constructor() {
    ...
    /**
     * Binds DOM listeners, configures Marked with Highlight.js, loads theme, and initializes history.
     */
    init() {
        // DOM Elements
    ...
    /**
     * Clears active message viewport and starts a fresh consultation thread.
     */
    createNewChat() {
    ...
    /**
     * Empties the message container and reveals the welcome suggestion cards.
     */
    clearChatDisplay() {
    ...
    /**
     * Updates the header consultation title display.
     * @param {string} title
     */
    updateHeader(title) {
    ...
    /**
     * Submits user input to the Flask /ask endpoint with the last 6 conversation turns for context.
     * Appends assistant answer and triggers localStorage persistence.
     */
    async sendMessage() {
    ...
    /**
     * Renders a message bubble to the viewport with avatars and risk keywords highlighted.
     *
     * @param {'user' | 'bot'} role - Author of message
     * @param {string} text - Message text with markdown formatting
     */
    ...
    /**
     * Returns HTML markup for assistant message action buttons (e.g. copy).
     * @returns {string}
     */
    getMessageActions() {
        return `
    ...
    /**
     * Persists conversation state to localStorage and re-renders sidebar history list.
     *
     * @param {string} userQuery - The question posed by the user
     * @param {string} botResponse - The answer generated by the assistant
     */
    ...
    /**
     * Groups consultation threads into chronological sections (Today, Yesterday, Older)
     * and filters results against search queries.
     *
     * @param {string} [filter=''] - Search term to match against title or message text
     */
    ...
    /**
     * Reconstitutes an existing conversation session from history into the active viewport.
     *
     * @param {string} id - Unique timestamp identifier of the chat
     */
    loadChat(id) {
    ...
    /**
     * Filters historical consultation sessions matching the search query.
     * @param {string} query
     */
    filterChats(query) {
    ...
    /**
     * Automatically expands textarea height according to content and toggles send button availability.
     */
    handleInput() {
    ...
    /**
     * Intercepts Enter keystrokes to trigger sendMessage while permitting Shift+Enter for newlines.
    ...
    /**
     * Controls the visibility of the typing loader animation.
    ...
    /**
     * Smoothly scrolls message viewport to the latest entry.
     */
    scrollToBottom() {
    ...
    /**
     * Opens or collapses mobile sidebar overlay.
     * @param {boolean} open
     */
    toggleSidebar(open) {
        if (open) {
    ...
    /**
     * Switches between light and dark visual themes.
     */
    toggleTheme() {
    ...
    /**
     * Populates input field with a suggested medical prompt and submits.
     * @param {string} text
     */
    ...
    /**
     * Copies message text from assistant bubble to clipboard.
